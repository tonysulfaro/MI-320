...
d = comic_characters %>%
  filter(year >= 1961) %>%
  mutate(male_count = nrow(subset(comic_characters, sex="Male"))) %>% 
  group_by(year, sex) %>%
  summarise(count = n()) %>%
...
comic_characters %>%
  ...
  summarise(count = n()) %>%
  ggplot(aes(x=align, y=count, color=sex)) + geom_line(stat = "identity",position="dodge")
```

#### Question 03

In	each	comic	universe,	do	male	characters	or	female	characters,	on	average,	have	
more	appearances	per	character,	just	looking	at	years	since	1961	and	characters	
that	appear	in	comics	often (more	than	100	appearances)?

```{r}
comic_characters %>%
  filter(year >= 1961) %>%
  group_by(year) %>%
  summarise(ml = nrow(subset(comic_characters, sex="Male"))) %>%
  ggplot(aes(x=year, y=ml)) + geom_line(stat = "identity",position="dodge")
```
