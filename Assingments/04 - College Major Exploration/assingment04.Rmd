...
The college data `college_all_ages` comes from the package `fivethrityeight` as a dataset included by defualt when installed. The data contained within the dataset originally came from an American Community Survey 2010-2012. The data was collected to help give insight into the compensation and unemployment rates. This data can help us answer questions about which majors have the higest and lowest compensation rates as well as how unemployment rates compare between major categories.

The data includes fields like major name, the major category it resides in (eg: Business), and some other numeric values like how many people are employed in that field, how many are unemployed, what the median salary is for that major, etc.

# Data Characteristics

There are `r nrow(college_all_ages)` entries in the `college_all_ages` dataset with `r ncol(college_all_ages)` variables. An observation in this dataset is a specific major and the attributes that go along with it (like number of people with that degree and median compensation for their field).
...
After generating the boxplot and arranging the major categories by average median salary we can see how much some of the categories vary. To get the average median salary I added another variable using the mutate command which is just the mean of the median salaries for each major category. 

Health appears to have a huge range of salaries, mostly due to an outlier at 100k+. Engineering and Industrial Arts & Consumer Services however have the widest boxes, indicating that there are majors with varying compensation amounts.
...
After looking through the various combinations of variable scatter plots I want to know a bit more about the correltion between median compensation and unemployment rate. Let's construct a scatterplot with a trend line on top to verify this information.
...
The negative correlation between salary and unemployment rate is shown in that the higher the compensation, the lower the unemployment rate tends to be. Some interesting exceptions to this trend appear to be "Education" and "Agriculture & natural Resources" which have the lowest unemployment rates out of the entire dataset.

# Report  

Overall, After doing some research on the college dataset I was able to learn some interesting things. Most namely which major categories have the highest percentage of full time workers. I had known previously that people involved in education were likely not to work full time but I thought it was interesting that at least 30% of those in Education worked full time (50 weeks @ 35 hours a week) during the year.
